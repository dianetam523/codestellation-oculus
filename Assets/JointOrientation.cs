...
<<<<<<< HEAD
// Making the fingers spread pose or pressing theA 'r' key resets the reference orientation.
public class JointOrientation : MonoBehaviour
{
	...
	// Current Velocity set to 25 (default)
	private double v = 25;

	// Max Velocity = 75
	private const int MAX_V = 75;

	//Min Velocity = 0
	private const int MIN_V = 0;
	...
	void Update ()
	{
		...
		if (thalmicMyo.xDirection == Thalmic.Myo.XDirection.TowardWrist) {
			// Mirror the rotation around t	he XZ plane in Unity's coordinate system (XY plane in Myo's coordinate
...
<<<<<<< HEAD
		...
		calcX = (double)transform.localRotation.x * 100.0;
...
=======
        ...
        double calcX = -1* (double)transform.localRotation.x * 100.0;
        ...
        Console.WriteLine("X= " + (double)transform.localRotation.x*100.0);
        Console.WriteLine("Y= " + (double)transform.localRotation.y * 100.0);
        Console.WriteLine("Z= " + (double)transform.localRotation.z * 100.0);
